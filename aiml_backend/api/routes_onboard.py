...
async def submit_onboarding(req: OnboardingSubmitRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Also we should create base skills for them based on aspirationFocus!
    skills_to_add = req.aspirationFocus if req.aspirationFocus else ["Mindset & Peace", "Career & Wealth"]
