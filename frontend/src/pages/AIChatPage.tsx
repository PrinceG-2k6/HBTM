...
export const AIChatPage: React.FC = () => {
  ...
  // Auto-scroll to bottom of messages container smoothly
    ...
    <div className="w-full max-w-5xl mx-auto space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <button
            onClick={clearChat}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-zinc-900/60 hover:bg-zinc-800 text-xs font-semibold text-zinc-400 hover:text-rose-400 transition-all cursor-pointer"
          >
      ...
      <div className="rounded-3xl bg-zinc-900/40 backdrop-blur-2xl shadow-2xl overflow-hidden flex flex-col h-[calc(100vh-200px)] min-h-[480px]">
        
        {/* Scrollable Messages Area */}
        <div
          ref={messagesContainerRef}
          className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-6 scrollbar-thin scrollbar-thumb-zinc-800"
        >
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {!isUser && (
                  <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-2xl bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-purple-900/30 mt-1">
                    <Sparkles size={16} />
                  </div>
                )}
                ...
                <div className={`max-w-[88%] sm:max-w-[80%] space-y-2 ${isUser ? "items-end" : "items-start"}`}>
                  {/* Message Bubble */}
                  <div
                    className={`rounded-3xl p-4 sm:p-5 shadow-xl transition-all ${
                      isUser
                  ...
                  >
                    ...
                    {msg.image && (
                      <div className="mb-3 overflow-hidden rounded-2xl max-w-xs sm:max-w-sm">
                        <img
                          src={msg.image}
                ...
                {isUser && (
                  <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-2xl bg-zinc-800 flex items-center justify-center text-zinc-300 shrink-0 mt-1">
                    <User size={16} />
                  </div>
                )}
              </div>
          ...
          {loading && (
            <div className="flex gap-3 items-start">
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-2xl bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center text-white shrink-0 shadow-lg animate-pulse">
                <Sparkles size={16} />
              </div>
        ...
        {/* Quick Prompts Bar */}
        <div className="px-4 py-2 bg-zinc-950/40 border-t border-white/5 flex items-center gap-2 overflow-x-auto no-scrollbar shrink-0">
          <Zap size={13} className="text-purple-400 shrink-0" />
          {QUICK_PROMPTS.map((qp, idx) => (
            <button
              key={idx}
              onClick={() => handleSend(qp)}
              className="text-xs font-medium px-3 py-1 rounded-full bg-zinc-900/80 hover:bg-purple-950/60 text-zinc-300 hover:text-purple-200 transition-all shrink-0 cursor-pointer"
            >
              {qp}
            </button>
          ))}
        ...
        {selectedImage && (
          <div className="px-4 py-2 bg-purple-950/30 flex items-center justify-between shrink-0">
        ...
        {/* Input Bar */}
        <div className="p-3 sm:p-4 bg-zinc-950/80 flex items-center gap-2.5 shrink-0">
          <input
            type="file"
            ref={fileInputRef}
          ...
          <input
            ...
            placeholder="Ask AI curator anything or upload image..."
            className="flex-1 bg-zinc-900/80 text-white placeholder-zinc-500 text-xs sm:text-sm px-4 py-3 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all"
          />

          <button
            onClick={() => handleSend()}
            disabled={!inputPrompt.trim() && !selectedImage}
            className="p-3 sm:p-3.5 rounded-2xl font-semibold text-white shadow-xl transition-all duration-200 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
            style={{
