...
export const SignInPage: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-[80vh] flex items-center justify-center py-6 px-3">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl text-white tracking-tight">Sign in to PACER</h1>
          <p className="text-xs text-zinc-400">
            Access your personalized agentic curation workspace
          </p>
        </div>
          ...
          {authMethod === "google" ? (
            <div className="space-y-4 py-2">
              <p className="text-xs text-zinc-400 text-center leading-relaxed">
                Sign in with one-click OAuth 2.0 Google Identity verification.
              </p>
              ...
              >
                ...
                <span>{loading ? "Authenticating Google OAuth..." : "Continue with Google Account"}</span>
              </button>

              <div className="p-3 rounded-2xl bg-zinc-900 border border-zinc-800 text-2xs text-zinc-400 space-y-1">
                <div className="flex items-center gap-1.5 text-emerald-400">
                  <CheckCircle2 size={12} /> Secure OAuth 2.0 Verification
                </div>
                <p>Google token auto-exchanges with PACER vector curator backend.</p>
              </div>
            </div>
          ) : (
            /* JWT Form Container */
            <form onSubmit={handleJWTSubmit} className="space-y-4">
              <div>
                <label className="text-2xs text-zinc-400 uppercase tracking-wider block mb-1.5">
                  Email Address
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="w-full text-xs pl-10 pr-4 py-3 rounded-2xl bg-zinc-900 border border-zinc-800 text-white placeholder-zinc-500 outline-none focus:border-amber-400 transition-colors"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-2xs text-zinc-400 uppercase tracking-wider block">
                    JWT Password
                  </label>
                  <a href="#forgot" onClick={(e) => e.preventDefault()} className="text-2xs text-amber-400 hover:underline">
                    Forgot password?
                  </a>
                </div>
                <div className="relative">
              ...
              >
                <span>{loading ? "Authenticating JWT..." : "Sign In to PACER"}</span>
                <ArrowRight size={14} />
              </button>
            </form>
          )}
